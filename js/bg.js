// Compatibility helper wrappers for Manifest V3 background Service Worker
...
chrome.browserAction = chrome.action || chrome.browserAction;
...
// Notification Click Handler Setup
var globalNotificationCallbacks = {};
if (chrome.notifications) {
	chrome.notifications.onClicked.addListener(function(notificationId) {
		if (globalNotificationCallbacks[notificationId]) {
			try {
				globalNotificationCallbacks[notificationId]();
			} catch(e) {}
			delete globalNotificationCallbacks[notificationId];
		}
...
function prepareSuggestions() {
	...
	suggestions = PREFiX.homeTimeline.buffered.
		...
		slice(0, 5).
		map(function(status) {
			var text = escapeHTML(status.textWithoutTags).
				...
				if (! settings.current.showSavedSearchCount) {
					...
					self.unread_count = self.statuses.filter(function(s) {
							s.is_unread = s.user.id !== PREFiX.account.id &&
								...
								s.textWithoutTags.indexOf('@' + PREFiX.account.name) === -1;
							return s.is_unread;
						}).length;
				}
			});
...
function showNotification(options) {
	...
	var notificationId = options.id || ('prefix_notification_' + Date.now() + '_' + Math.random());
	...
	var mockNotification = {
		...
		addEventListener: function(event, callback) {
			if (event === 'click') {
				globalNotificationCallbacks[notificationId] = callback;
			} else if (event === 'close') {
	...
	if (options.timeout !== false) {
		...
		mockNotification.timeout = setTimeout(function() {
			hideNotification(mockNotification);
		}, timeoutMs);
	}
...
var playSound = (function() {
	...
	return function(force) {
		...
		timeout = setTimeout(function() {
			...
			if (typeof Audio === 'undefined') {
				if (globalThis.playAudioViaOffscreen) {
					globalThis.playAudioViaOffscreen('dongdong.mp3', volume);
				}
			} else {
...
Ripple.events.observe('process_status', function processStatus(status) {
	...
	if (status.source) {
		status.source = stripHTML(status.source);
		status.source = ({
			'网页': 'Web',
			'手机上网': 'Wap',
			'iPhone版': 'iPhone'
...
var PREFiX = this.PREFiX = {
	...
	user: lscache.get('access_token') ? Ripple(lscache.get('access_token')) : null // 一个 Ripple 实例, 提供所有 API 接口
};

var is_first_run = false;
var startup = true;

if (typeof isServiceWorker === 'undefined') {
	initialize();
	is_first_run = lscache.get('is_first_run') !== false;
